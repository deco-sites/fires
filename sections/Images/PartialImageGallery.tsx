...
import Icon, { AvailableIcons } from "deco-sites/fires/components/ui/Icon.tsx"
...
export default function PartialImageGallery({
    ...
    return (
        <section class="p-4 mx-auto flex flex-col gap-4 max-w-screen-xl items-center">
            ...
            <ul class="flex gap-2 mb-2 flex">
                {shownImages.map((pageImages: Image) => (
                    <li class="w-full md:max-w-80 flex">
                        <Image class="max-w-[350px] w-full h-auto rounded-md hover:scale-125 transition-all hover:shadow-md cursor-pointer object-cover" width={334} height={200} src={pageImages.src} alt={pageImages.alt} />
                    </li>
                ))}
            </ul>
