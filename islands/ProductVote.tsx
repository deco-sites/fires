...
import { invoke } from "deco-sites/fires/runtime.ts";
import { Bounce, toast, ToastContainer, TypeOptions } from "react-toastify";
import { sendEvent } from "deco-sites/fires/sdk/analytics.tsx";
import Icon from "deco-sites/fires/components/ui/Icon.tsx";
...
export default function ProductVote({ productId }: Props) {
    ...
    useSignalEffect(() => {
        const getVotes = async () => {
            const votesTotalProduct = await invoke["deco-sites/fires"].loaders.votesProduct({ productId });
            productVotes.value = votesTotalProduct.product;
 
            const votesTotal = await invoke["deco-sites/fires"].loaders.votesTotal();
    ...
    const addVote = async () => {
        if (hasVoted.value !== true) {
            hasVoted.value = true;
            const vote = await invoke["deco-sites/fires"].actions.sendVote({
                productId,
            });
